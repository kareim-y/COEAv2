...
from get_well_data import get_formation_well_data  # Kareem notes: missing file??
...
from Spatial_impact_plotting import plot_water_shapefiles # Kareem notes: missing file??
...
def induced_seismic_assessment(well_data_headings, well_data):
	...
	count = 0
	max_dist = float(input('What is the maximum distance of investigation (km)? (Distance between well and seismic event):   '))
	min_mag = float(input('What is the minimum magnitude of the event?   '))
	max_days_between = float(input('The maximum amount of time between completion date and event (days):   '))
	negative_days =  str(input('Include Seismic Events that Happen in an Area Prior to Drilling (Yes/No)?  :   '))
	...
	plot_YN = input('Would you like to plot the events (Yes/No)?   ')
