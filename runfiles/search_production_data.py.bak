...
def search_production_data(well_data):
	...
	print('The database was exported from geoSCOUT January 2020')
	print('Getting Production Data For Selected Wells')
	print('\nData is Loading, May Take a Minute.....\n')

	start_time = time.time()

	prod_data = collections.OrderedDict()
		...
		for row in reader:
			...
			if row[0] == 'Unique Well ID':
				...
				if well_ID in well_data:
					in_well = 'yes'
					switch = 0
					#prod_data[well_ID].append(well_ID)
			...
			if (switch == 1 and in_well == 'yes'):
				try:
					prod_data[well_ID].append(row)
				except:
					prod_data[well_ID] = [row]
	...
	for well in prod_data:
		count2 = count2 + 1

	print(str(count2) + ' Of The Selected Wells Have Been Found With Production Data')
	print('\n')
	#print(get_size(prod_data))
	#print(get_size(well_data))

	time.sleep(5)

	return production_data_headings, prod_data, well_data_headings, well_header_data 
...
if __name__ == '__main__':
	...
	search_production_data(well_data)