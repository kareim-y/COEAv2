...
def plot_shapefile(shape_data,shape_name,count):
	...
	if shape_name == 'Duvernay':
		...
		for (i, j) in zip(intervals[:-1], intervals[1:]):
			...
			if counter in [1,11]:
				plt.fill(*list(zip(*points[i:j])),color=get_formation_color(shape_name, fill_map_type), alpha = alpha)
				plt.plot(*list(zip(*points[i:j])), label=shape_name, color = get_formation_color(shape_name, outline_map_type), linewidth = line_width)
			counter +=1
	...
	by_label = collections.OrderedDict(list(zip(labels, handles)))
	leg = plt.legend(list(by_label.values()), list(by_label.keys()), markerscale=3, bbox_to_anchor=(0.93, 0.95), fontsize=12)
