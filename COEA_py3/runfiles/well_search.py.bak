...
def well_search():
	...
	#Get date limits
	date_min = str(raw_input('Enter a Drilled After Date (DD/MM/YYYY): '))
	date_min = datetime.strptime(date_min, '%d/%m/%Y')
	date_max = str(raw_input('Enter a Drilled Before Date (DD/MM/YYYY): '))
	date_max = datetime.strptime(date_max, '%d/%m/%Y')

	#get province of interest
	provinces =  str(raw_input('Enter provinces of Interest (separate by , ). AB,BC and SK available;  '))
	...
	while formations.upper() == 'SEARCH':
		formations = str(raw_input('Enter formations of Interest (separate by , ). Type "Search" for a complete list of formations;  '))
	...
	horizontal_TF = str(raw_input('Horizontal Well? (True, False or Both): '))
	...
	min_GOR= float(raw_input('Enter a Minimum First 12 month Ave GOR (m3/m3):  '))
